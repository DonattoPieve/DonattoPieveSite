...
const en: Dict = {
  ...
  "proj.github.desc":
    "Studies, prototypes and experiments — open source and always growing.",
...
export function LangProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("dp-lang");
...
export function LangToggle() {
  ...
  return (
    <button
      ...
      className="flex h-8 w-8 items-center justify-center rounded-full text-xs font-semibold text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground"
    >
      {next.toUpperCase()}
    </button>
  );
